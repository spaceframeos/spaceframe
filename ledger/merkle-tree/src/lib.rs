...
mod tests {
    ...
    fn test_merkle_with_transactions() {
        ...
        assert_eq!(Hash::hash(b"data1"), merkle.tree[0]);
        assert_eq!(Hash::hash(b"data2"), merkle.tree[1]);
        assert_eq!(Hash::hash(b"data3"), merkle.tree[2]);
        assert_eq!(Hash::hash(b"data4"), merkle.tree[3]);

        assert_eq!(
            hex::decode("a6b764089d73a35323f5bf570e3bc8a803c78953cafe9ff4297233b2c9bc24ba")
                .unwrap(),
            merkle.root().unwrap().to_vec()
        );
    }
    ...
    fn test_merkle_add_one_element() {
        ...
        assert_eq!(Hash::hash(b"data"), merkle.tree[0]);
    }
    ...
    fn test_merkle_add_five_elements() {
        ...
        assert_eq!(Hash::hash(b"data3"), merkle.tree[2]);
        assert_eq!(Hash::hash(b"data4"), merkle.tree[3]);
        assert_eq!(Hash::hash(b"data5"), merkle.tree[4]);
    }
    ...
    fn test_merkle_root() {
        ...
        assert_eq!(
            hex::decode("a6b764089d73a35323f5bf570e3bc8a803c78953cafe9ff4297233b2c9bc24ba")
                .unwrap(),
            merkle_tree.root().unwrap().to_vec()
        );
    }
}
